...
1. **Enter identity and marks.** Type the student's **name** and **roll number**, then the five subject marks — **Maths, Science, English, History, Computer** — into the **form inputs** at the top of the page [Readme.md:L46-L53]. Blank or non-numeric marks are accepted and treated as `0` (the **no-NaN guard**, explained under [Troubleshooting](#troubleshooting)). See [`../functionality/data-entry.md`](../functionality/data-entry.md) for identity capture (**F-001**) and marks entry (**F-002**).
...
| **A subject's marks are treated as `0`.** | A blank or non-numeric mark defaults to `0` via the **no-NaN guard** `parseInt(document.getElementById('<id>').value \|\| 0)` [Readme.md:L167-L171]: a blank number field has `.value === ""` (falsy), which becomes `0` *before* `parseInt` runs, so the field contributes `0` rather than `NaN`. | Enter a numeric value for every subject you intend to count. See [`../functionality/data-entry.md`](../functionality/data-entry.md). |
