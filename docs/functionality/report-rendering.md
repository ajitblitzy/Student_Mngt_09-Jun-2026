...
- **Report-card DOM region (static markup):** [Readme.md:L59-L78]
- **Rendering logic (marks-table rebuild + result writes):** [Readme.md:L176-L212]

The markup that receives the rendered output lives in the `index.html` block [Readme.md:L59-L78],
and the JavaScript that populates it runs at the end of `generateReport()` in the `script.js`
block [Readme.md:L176-L212].
...
the **report-card** region of the page [Readme.md:L176-L212].

### The report-card region

The report card is a single container `<div id="reportCard">` that is **always present** in the
page markup [Readme.md:L59]. Inside it are the **report-card placeholders** that JavaScript fills
in — five `<span>` elements (`#rName`, `#rRoll`, `#totalMarks`, `#percentage`, `#grade`) and a
single `<tbody id="marksTable">` for the per-subject rows [Readme.md:L62-L77]:
...
[Readme.md:L60], the surrounding `<table>` and its `<thead>` column headers `Subject` and `Marks`
[Readme.md:L65-L71], and the literal `%` sign printed after `#percentage` [Readme.md:L76]. For the
...
[Readme.md:L176], then **clears it** by assigning an empty string to `innerHTML` [Readme.md:L177],
and finally appends one `<tr>` row per subject inside the `for...in` loop [Readme.md:L179-L190]:

```javascript
const tableBody = document.getElementById('marksTable');  // L176
tableBody.innerHTML = '';                                  // L177 — clear (rebuild-from-scratch)
// ...
tableBody.innerHTML += row;                                // L189 — append one <tr> per subject
```

Because the `subjects` object holds exactly five keys [Readme.md:L166-L172], the loop appends
**exactly five rows** — one per subject — into the otherwise-static `<table>` [Readme.md:L179-L190].
Only the `<tbody id="marksTable">` content is rebuilt; the `<table>` and `<thead>` markup
(including the `Subject`/`Marks` headers) are never touched by JavaScript [Readme.md:L65-L72].
...
```javascript
document.getElementById('rName').innerText = name;                       // L208
document.getElementById('rRoll').innerText = roll;                       // L209
document.getElementById('totalMarks').innerText = total;                 // L210 — unformatted
document.getElementById('percentage').innerText = percentage.toFixed(2); // L211 — 2-decimal string
document.getElementById('grade').innerText = grade;                      // L212
```

Two precision details matter here:

- `#totalMarks` is written **unformatted** — the raw numeric `total` is assigned directly to
  `.innerText` [Readme.md:L210].
- `#percentage` is written as a **two-decimal string** via `.toFixed(2)` [Readme.md:L211]; the
  trailing `%` the reader sees is the static markup at [Readme.md:L76], **not** produced by this
  write.
...
run **clears** the marks-table body with `tableBody.innerHTML = ''` [Readme.md:L177] *before* the
per-subject loop repopulates it [Readme.md:L179-L190]. As a result, the rendered table always
contains **exactly five fresh rows** (one per subject) and **never accumulates stale or duplicate
rows** from previous runs [Readme.md:L176-L190].

Without the clear at [Readme.md:L177], repeated **Generate Report** clicks would append duplicate
...
|---|---|---|
| **Trigger** | Runs at the end of `generateReport()`, after `total`, `percentage`, and `grade` are computed. | [Readme.md:L176-L212] |
| **Writes — name** | `#rName.innerText = name` (the student name). | [Readme.md:L208] |
| **Writes — roll** | `#rRoll.innerText = roll` (the roll number). | [Readme.md:L209] |
| **Writes — total** | `#totalMarks.innerText = total`, written **unformatted**. | [Readme.md:L210] |
| **Writes — percentage** | `#percentage.innerText = percentage.toFixed(2)`, a **two-decimal string**; the trailing `%` is static markup. | [Readme.md:L211], [Readme.md:L76] |
| **Writes — grade** | `#grade.innerText = grade`. | [Readme.md:L212] |
| **Writes — marks table** | `<tbody id="marksTable">` rebuilt to **exactly five rows**, one per subject. | [Readme.md:L176-L190] |
| **Rebuild-from-scratch** | The table body is cleared before repopulation → no stale rows across runs; the report card reflects the latest inputs. | [Readme.md:L177] |
| **Determinism** | Re-running with identical **form inputs** produces an identical report card (no randomness, time, or persistence). | [Readme.md:L162-L213] |
| **Side effects** | Mutates the **rendered DOM** only — no persistence, no network; `#reportCard` is always present and is never hidden or shown. | [Readme.md:L59-L78], [Readme.md:L208-L212] |

**Preconditions.** The six report-card IDs — `#rName`, `#rRoll`, `#marksTable`, `#totalMarks`,
`#percentage`, `#grade` — must exist exactly as spelled in the markup [Readme.md:L62-L77]; the full
...
**form inputs** [Readme.md:L220-L224]. This is why **Generate Report must run before Download PDF**.
