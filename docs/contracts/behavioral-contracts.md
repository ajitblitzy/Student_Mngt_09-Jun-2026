...
The behavior described here lives in the embedded `script.js` block [Readme.md:L176-L253],
...
- `generateReport()` — reads the **form inputs**, computes the result, and writes the
  **rendered DOM** report card [Readme.md:L177-L228].
- `downloadPDF()` — reads the **rendered DOM** report card and saves a PDF [Readme.md:L230-L252].
...
|---|---|---|---|
| 1 | Rebuild-from-scratch rendering | The marks table is cleared before re-population, so each run yields a fresh table of exactly five rows. | [Readme.md:L191-L192] |
| 2 | No-NaN guard | `parseInt(... .value \|\| 0)` defaults a blank/falsy mark field to `0` before parsing, so an empty field contributes `0` rather than `NaN`. | [Readme.md:L182-L186] |
| 3 | Fixed 2-decimal percentage precision | The *displayed* percentage is a 2-decimal string via `.toFixed(2)`; the internal value stays a full-precision number. | [Readme.md:L226] |
| 4 | Default grade `'F'` | `grade` is initialized to `'F'` and retained whenever every threshold test fails (percentage `< 50`). | [Readme.md:L209] |
| 5 | DOM-read invariant | `downloadPDF()` reads from the **rendered DOM** report card, **not** from the **form inputs**. | [Readme.md:L235-L239] |
...
its contents [Readme.md:L191-L192]:
...
[Readme.md:L194-L205]. Because the table is emptied first, **rebuild-from-scratch** guarantees
...
`parseInt(...)` call [Readme.md:L182-L186]:
...
never `NaN` from an empty field [Readme.md:L181-L187]. Note the precise scope of this guard:
...
DOM **without** any formatting [Readme.md:L225]:
...
The grade variable is initialized to `'F'` before any threshold is tested [Readme.md:L209]:

```javascript
let grade = 'F';
```

`grade` is then potentially reassigned by an ordered `if / else-if` cascade [Readme.md:L211-L221].
...
(`#studentName`, `#rollNumber`, `#maths`, …) [Readme.md:L235-L239]:
...
|---|---|---|
| **Ordering: Generate Report before Download PDF** | `downloadPDF()` reads the **rendered DOM** report card, so that report card must already be populated by a prior `generateReport()` run. Otherwise the rendered fields are empty/default and the PDF contains blank values. | [Readme.md:L235-L239] |
| **jsPDF present** | `downloadPDF()` destructures the global `window.jspdf`; the cdnjs `<script>` must have loaded and populated that global before the function runs. | [Readme.md:L53], [Readme.md:L231] |

**Ordering.** The user must click **Generate Report** before **Download PDF**. This is a direct
consequence of the [DOM-read invariant](#5-dom-read-invariant) [Readme.md:L235-L239]: the PDF is
...
`const { jsPDF } = window.jspdf;` [Readme.md:L231], the global `window.jspdf` must have been
populated by the CDN script tag [Readme.md:L53]:
...
|---|---|---|
| `generateReport()` | The report card is populated: `#rName`, `#rRoll`, `#totalMarks`, `#percentage`, and `#grade` are all written. | [Readme.md:L223-L227] |
| `generateReport()` | The marks table contains **exactly five rows** — one per subject. | [Readme.md:L194-L205] |
| `downloadPDF()` | A PDF is generated and downloaded, named ``${name}_Report.pdf``. | [Readme.md:L251] |
| `downloadPDF()` | The PDF content is populated from the **rendered DOM** values, not the form inputs. | [Readme.md:L235-L249] |

After `generateReport()`, all five report-card fields are written in a single pass
[Readme.md:L223-L227], and the marks table holds exactly five rows because of the
[rebuild-from-scratch invariant](#1-rebuild-from-scratch-rendering) [Readme.md:L194-L205].

After `downloadPDF()`, the browser saves a file whose name is derived from the rendered name
field [Readme.md:L251]:

```javascript
doc.save(`${name}_Report.pdf`);
```

Here `name` is read from the rendered `#rName` element [Readme.md:L235], so the filename
...
- **Missing jsPDF → `TypeError`.** If `window.jspdf` is `undefined` — the CDN is unreachable,
  ...
  destructuring line [Readme.md:L231] throws a `TypeError` (cannot destructure property
...
- **No try/catch, no input validation.** There is **no** `try/catch` anywhere and **no** input
  validation beyond the [no-NaN guard](#2-no-nan-guard) (`|| 0`) [Readme.md:L176-L253].
...
- **Download before Generate → blank PDF (no error thrown).** If **Download PDF** is clicked
  ...
  populated, so the [DOM-read invariant](#5-dom-read-invariant) [Readme.md:L235-L239] yields
...
only the report card [Readme.md:L177-L228] — there is no randomness, no `Date`/time dependency,
...
grade. The ordered `if / else-if` cascade [Readme.md:L211-L221] tests the highest band first,
...
the source. The `'F'` default [Readme.md:L209] catches every percentage below `50`. Together
...
flowchart TD
    ...
    D -- "No" --> E["TypeError: cannot destructure<br/>'jsPDF' of undefined<br/>(Readme.md:L231)"]
    D -- "Yes" --> F["PDF generated from rendered DOM<br/>saved as &lt;name&gt;_Report.pdf<br/>(Readme.md:L251)"]
```
